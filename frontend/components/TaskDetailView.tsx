...
export const TaskDetailView: React.FC<TaskDetailViewProps> = ({ 
  ...
  // Derived stats
  const distressCount = records.filter(r => r.pavementDistress && r.pavementDistress.length > 0).length;
  
  const avgIri = records.length > 0 
  ...
  return (
    <PullToRefresh onRefresh={onRefresh} isRefreshing={isRefreshing} refreshResult={refreshResult}>
      <div className="p-4 pb-20 max-w-lg mx-auto w-full">
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
              ...
              <div className="bg-white p-3 rounded-lg border border-slate-100 shadow-sm flex flex-col items-center">
                  ...
                  <span className={`text-lg font-bold ${Number(avgIri) > 4 ? 'text-red-600' : 'text-slate-800'}`}>{avgIri}</span>
              </div>
              <div className="bg-white p-3 rounded-lg border border-slate-100 shadow-sm flex flex-col items-center">
                  <span className="text-xs text-slate-400 uppercase">病害数</span>
                  <span className="text-lg font-bold text-slate-800">{distressCount}</span>
              </div>
          </div>
            ...
            {!task.isFinished && (
              <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-200">
                  <div className="max-w-lg mx-auto flex gap-3">
                        ...
                        <button className="flex-1 bg-blue-600 text-white py-3 rounded-lg font-semibold text-sm shadow-lg shadow-blue-500/30">
                          继续巡检
                        </button>
                  </div>
              </div>
